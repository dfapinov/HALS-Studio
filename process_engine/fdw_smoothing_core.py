...
def apply_complex_smoothing(freqs, complex_data, oct_res, t_peak_sec):
    oct_res = np.asarray(oct_res, dtype=float)
    if np.any(~np.isfinite(oct_res)) or np.any(oct_res <= 0): raise ValueError('Smoothing resolution must be positive and finite.')
    ...
    for i in range(n_bins):
        ...
        bw_hz = f_target * (k_factor if k_factor.ndim == 0 else k_factor[i])
