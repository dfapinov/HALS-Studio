...
2. Sound Power: Normalized Appendix C Table 7 solid-angle weights.
3. Averaging Domains: Energy for all spatial composites.
"""
...
def calculate_cta2034_energy_metrics(energy, indices):
    """Shared Analysis/Export CTA-2034-A calculations from squared pressure.

    Uses section 5.2's five equally weighted reflection groups, Appendix C
    Table 7 sound-power weights and the 12/44/44 predicted in-room mixture.
    The ER rear group is three directions; the separate horizontal-reflections
    diagnostic uses nineteen rear directions. All spatial averages use energy.
    See documentation/cta2034_methodology.md for the reference and conventions.
    """
    ...
    # Front/rear are shared by both orbits; count each only once.
...
def calculate_cta2034_metrics(freqs, p_matrix, map_idx, coord_deviations):
    """FRD naming/phase adapter for the shared energy calculation.

    Composite FRDs retain on-axis phase for compatibility; it is not a
    coherently averaged phase. DI has zero phase. Geometry metadata and
    frequencies remain in the signature for existing Stage 5 callers.
    """
    levels = calculate_cta2034_energy_metrics(np.abs(p_matrix)**2, map_idx)
    ph_ref = np.angle(p_matrix[:, map_idx['H0']], deg=True)
    names = {
        'Response_OnAxis': 'On axis',
        'Response_ListeningWindow': 'Listening window',
        'Response_EarlyReflections': 'Early reflections',
        'Response_SoundPower': 'Sound power',
        'Response_PIR': 'Predicted in-room',
